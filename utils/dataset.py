...
class PT_DatitaSet(torch.utils.data.Dataset):
    def __init__(self, mode, n_classes, img_dim, patch_size):
        super().__init__()
        self.mode = mode
        self.n_classes = n_classes
        self.img_dim = img_dim
        ...
        img_paths, labels = self.get_paths()
        self.transform = img_transforms[self.mode]
        patches, labels = self.transform_images(img_paths, labels)
...
def main(mode, n_classes, img_dim):
    if mode == "pretrained":
